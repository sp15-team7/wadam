...
export const wineDetailReviewSchema = reviewResponseSchema.extend({
  lightBold: nonNegativeNumberSchema.min(1).max(10),
  smoothTannic: nonNegativeNumberSchema.min(1).max(10),
  drySweet: nonNegativeNumberSchema.min(1).max(10),
  softAcidic: nonNegativeNumberSchema.min(1).max(10),
  isLiked: z.boolean(),
});
